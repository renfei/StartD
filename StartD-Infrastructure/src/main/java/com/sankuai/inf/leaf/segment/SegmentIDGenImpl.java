...
public class SegmentIDGenImpl implements IDGen {
    ...
    private List<String> getAllTags() {
        ...
        List<LeafAlloc> leafAllocs = mapper.selectByExample(example);
        if (leafAllocs != null && leafAllocs.size() > 0) {
            List<String> list = new CopyOnWriteArrayList<>();
            leafAllocs.forEach(obj -> list.add(obj.getBizTag()));
            return list;
        } else {
    ...
    private LeafAlloc updateMaxIdByCustomStepAndGetLeafAlloc(LeafAlloc temp) {
        mapper.updateMaxIdByCustomStep(temp);
        return mapper.selectByPrimaryKey(temp.getBizTag());
    }
    ...
    public LeafAllocMapper getMapper() {
        return mapper;
    }

    public void setMapper(LeafAllocMapper mapper) {
        this.mapper = mapper;
    }
}
